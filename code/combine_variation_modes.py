...
def get_combine_variation_modes(shape_mean, shape_eig_values, shape_eig_vecs, texture_mean, texture_eig_values,
	texture_eig_vecs, save_plot_dir='/content/myresualt/faces/combine_modes'):

	os.makedirs(save_plot_dir, exist_ok=True)

	shape_mean_img_path = os.path.join('/content/drive/MyDrive/data_rgb', '383.jpg') # From shape modeling (Hard-coded)
	...
	for s, shape_mode in enumerate([shape_var_1_minus, shape_var_1_plus, shape_var_2_minus, shape_var_2_plus]):
		for t, texture_mode in enumerate([texture_var_1_minus, texture_var_1_plus, texture_var_2_minus, texture_var_2_plus]):
			combine_mode = combine_shape_with_normalized_texture(shape_mode, cv2.resize(texture_mode.squeeze(0),
				(texture_var_1_plus.shape[1]*4, texture_var_1_plus.shape[2]*4) ), shape_mean_x_coord, shape_mean_y_coord, return_shape=True)

			axarr[s, t].imshow(combine_mode[0] , cmap='gray')
			axarr[s, t].axis('off')
...
if __name__ == '__main__':
	...
	shape_mean = np.load('/content/myresualt/shape_mean.npy')
	shape_eig_values = np.load('/content/myresualt/shape_eigvalues.npy')
	shape_eig_vecs = np.load('/content/myresualt/shape_eigvecs.npy')

	texture_mean = np.load('/content/myresualt/texture_mean.npy')
	texture_eig_values = np.load('/content/myresualt/texture_eigvalues.npy')
	texture_eig_vecs = np.load('/content/myresualt/texture_eigvecs.npy')
